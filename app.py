...
def check_and_see(warehouse, date):
    ...
    st.sidebar.write(
        f"Taux de remplissage moyen sur tous les trajets : **{sub_routes.loc[:, 'fill_rate'].mean()*100:.0f}%**"
    )
    ...
    for k in range(len(sub_routes)):
        ...
        sub_cities = pd.merge(
            sub_orders[["delivery_location", "order_total_volume"]]
        ...
        gdf_cities = gpd.GeoDataFrame(
            sub_cities,
            geometry=gpd.points_from_xy(sub_cities.lng, sub_cities.lat),
            crs=4326,
        )
        gdf_cities.to_crs(epsg=3857).plot(
            ax=ax, column="order_total_volume", markersize=200
        )  # , legend=True)
...
warehouse = st.sidebar.selectbox(
    "Choisir un entrepôt", options=warehouses.warehouse_city.tolist()
)
date = st.sidebar.selectbox(
    "Choisir une date de livraison", options=orders.delivered_date.unique().tolist()
)
